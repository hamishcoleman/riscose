...
/*  register ARMword dest;       almost the DestBus */
/*  register ARMword temp;       ubiquitous third hand */
  register ARMword pc;		/* the address of the current instruction */
/*  ARMword lhs, rhs;            almost the ABus and BBus */
  ARMword decoded, loaded;	/* instruction pipeline */
  ARMword cond;			/* is the condition code true */
...
\***************************************************************************/
  ...
  do
	...
	{
	case SEQ:		/* Advance the pipeline, and an S cycle */
	  state->Reg[15] += 4;
	  pc += 4;
	  instr = decoded;
	  decoded = loaded;
	...
	case NONSEQ:		/* Advance the pipeline, and an N cycle */
	  state->Reg[15] += 4;
	  pc += 4;
	  instr = decoded;
	  decoded = loaded;
	...
	case PCINCEDSEQ:	/* Program counter advanced, and an S cycle */
	  pc += 4;
	  instr = decoded;
	  decoded = loaded;
	  loaded = ARMul_LoadInstrS (state, pc + 8);
	...
	case PCINCEDNONSEQ:	/* Program counter advanced, and an N cycle */
	  pc += 4;
	  instr = decoded;
	  decoded = loaded;
	  loaded = ARMul_LoadInstrN (state, pc + 8);
...
\***************************************************************************/
	...
	switch ((int) TOPBITS (28))	/* check the condition code */
	  ...
	  case LS:
	    ...
	    break;
	  case GE:
	    cond = ((!NFLAG && !VFLAG) || (NFLAG && VFLAG));
	    break;
	  case LT:
	    cond = ((NFLAG && !VFLAG) || (!NFLAG && VFLAG));
	    break;
	  case GT:
	    cond = ((!NFLAG && !VFLAG && !ZFLAG) || (NFLAG && VFLAG && !ZFLAG));
	    break;
	  case LE:
	    cond = ((NFLAG && !VFLAG) || (!NFLAG && VFLAG)) || ZFLAG;
	    break;
	  }			/* cc check */
...
\***************************************************************************/
      ...
      if (cond)			/* if the condition codes don't match, stop here */
	{
	    ...
	    case 0xb7:
#ifdef ARMUL_SHOW_PROGRESS
              {
               ...
               if ( (x & 0xffffff00) == 0xff000000)
                 {
                  lbl = mem_chunk(pc+4+POSBRANCH - (x & 0xff), 0);
                  printf("Branched to label %s\n", lbl);
                 }
              }
#endif
#ifdef MODE32
...
\***************************************************************************/
	    ...
	    case 0xbf:
#ifdef ARMUL_SHOW_PROGRESS
              {
               ...
               if ( (x & 0xffffff00) == 0xff000000)
                 {
                  lbl = mem_chunk(pc+4+NEGBRANCH - (x & 0xff), 0);
                  printf("Branched to label %s\n", lbl);
                 }
              }
#endif
...
\***************************************************************************/
	    ...
	    case 0xef:
	      if (BIT (4))
		{		/* MRC */
		  ARMword temp = ARMul_MRC (state, instr);
		  if (DESTReg == 15)
		    {
		      ASSIGNN ((temp & NBIT) != 0);
		      ASSIGNZ ((temp & ZBIT) != 0);
...
static unsigned
LoadByte (ARMul_State * state, ARMword instr, ARMword address)
{
  ARMword dest;
...
static void
LoadMult (ARMul_State * state, ARMword instr,
